 ...
 * Movement class
 *
 * a helper class for controlling the movement of the player and ghosts
 */

[RequireComponent(typeof(Rigidbody))]
public class Movement : MonoBehaviour
{
    public float speed = 8f; // basic speed
    public float speedMultiplier = 1f; // speed multiplier is affected by powerups or any special effects
    public Vector3 initialDirection;

    public new Rigidbody rigidbody { get; private set; }
    public Animator animator { get; private set; } // pacman animator
    ...
    private void Awake() {
        ...
        try {animator = transform.GetChild(0).GetChild(0).GetComponent<Animator>();} // to get pacman animator
    ...
    private void OnGameStart() {
        ...
        ResetState(); // reset state when game starts
    }
    ...
    private void Update() {
        // keep updating the direction
        if (nextDirection != Vector3.zero) {
            SetDirection(nextDirection);
        }
        
        // if the player is blocked, set the next direction and stop the animation
        if (isBlocked(direction) && gameObject.CompareTag("Pacman")) {
            if (animator != null) animator.enabled = false;
        }
        else if (gameObject.CompareTag("Pacman")) { // pacman is not blocked
            if (animator != null) animator.enabled = true;
        }
    }
    
    // Fixed Update is used for any physics related updates
    ...
    public void SetDirection(Vector3 targetDir, bool forced = false) {
        ...
        if (isBlocked(targetDir) && !forced) {
            nextDirection = targetDir; // set nextDirection to targetDir if it is blocked
            // Debug.Log("Path is blocked. Setting nextDirection to: " + nextDirection.ToString());
        }
        else {
            nextDirection = targetDir;
            direction = targetDir; // set direction to targetDir if it is not blocked
    ...
    private float rayOffset = 0.45f; // offset for raycast
    public bool isBlocked(Vector3 testDirection) {
        
        // use 4 raycasts to check if the path is blocked
        ...
        // RaycastHit stores information about the raycast hit point
        RaycastHit forwardHit;
        RaycastHit leftHit;
        RaycastHit rightHit;
        RaycastHit backHit;
        
        // set the raycast distance to 0.5f
